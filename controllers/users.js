...
export const register = async (req, res) => {
  ...
  if (!password) {
    return res.status(400).json({ success: false, message: '缺少密碼欄位' })
  }
  if (password.length < 4) {
    return res
      .status(400)
      .json({ success: false, message: '密碼必須 4 個字以上' })
  }
  if (password.length > 20) {
    return res
      .status(400)
      .json({ success: false, message: '密碼必須 20 個字以下' })
  }
  if (!password.match(/^[A-Za-z0-9]+$/)) {
    return res.status(400).json({ success: false, message: '密碼格式錯誤' })
  }
